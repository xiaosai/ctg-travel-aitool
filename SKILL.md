---
name: travel-project
description: 旅游项目用户交互与资源订购 Skill。覆盖火车、机票、酒店、门票四大资源线。当用户输入「购买火车票」「购买机票」等明确订购需求时，在平台完成购票/预订；当用户输入「我想去XX地方旅游」等模糊需求时，先进行旅游地推荐，再引导用户在平台完成资源订购。支持多资源同时订购时逐个引导、依次下单。
---
...
| 接口 | 缓存文件 | 过期时间 |
|------|----------|----------|
| `cityList` | `cache/cityList_{resourceType}.json` | 1 小时 |

**详细实现**：见 [cache_implementation.md](cache_implementation.md)
