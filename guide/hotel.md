...
**创建订单前，必须向用户展示并确认入住人信息，不得直接下单！**

正确流程：
1. 获取入住人列表后，向用户展示：「为您找到以下入住人：① 张三 ② 李四，请确认使用哪位入住人预订？」
2. 用户确认后，再调用 `hotel.createOrder` 创建订单
3. 如果用户需要更换入住人，引导用户先新增入住人

❌ **错误做法**：获取入住人列表后直接调用 `createOrder`
✅ **正确做法**：展示入住人 → 用户确认 → 再下单
